...
- [ ] **CLOS-01**: User can see month-end close progress — 6 close stages (AP close, AR close, Revenue recognition, Inventory valuation, Accruals & manual JEs, Financial statement package) display progress bars computed from actual journal entry data counts
- [ ] **CLOS-02**: User can see close health at a glance — each stage shows a RAG status badge (On Track / At Risk / Delayed) with an Iconsax icon, color-coded and visually prominent
- [ ] **CLOS-03**: User can understand why a stage is at risk — at-risk stages display a contextual note derived from JE data (e.g., "12 of 15 expected JEs posted — 3 pending approval")
